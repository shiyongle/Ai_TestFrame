...
from models.database_models import UIAutomationArtifact, UIAutomationStepLog, UIAutomationTask, UIAutomationCase, SystemSetting
from schemas.ui_automation_schemas import UIAutomationCaseCreate, UIAutomationTaskCreate
...
class UIAutomationService:
    ...
    def list_cases(self, db: Session, project_id: Optional[int] = None) -> List[UIAutomationCase]:
        query = db.query(UIAutomationCase)
        if project_id is not None:
            query = query.filter(UIAutomationCase.project_id == project_id)
        return query.order_by(UIAutomationCase.updated_at.desc(), UIAutomationCase.id.desc()).all()

    def get_case(self, db: Session, case_id: int) -> Optional[UIAutomationCase]:
        return db.query(UIAutomationCase).filter(UIAutomationCase.id == case_id).first()

    def create_case(self, db: Session, payload: UIAutomationCaseCreate) -> UIAutomationCase:
        case = UIAutomationCase(
            name=payload.name,
            ...
            natural_language_steps=payload.natural_language_steps or [],
    ...
    def update_case(self, db: Session, case_id: int, payload: Dict[str, object]) -> Optional[UIAutomationCase]:
    ...
    def _create_task_entity(self, db: Session, payload: UIAutomationTaskCreate) -> UIAutomationTask:
    ...
    def create_task(self, db: Session, payload: UIAutomationTaskCreate) -> UIAutomationTask:
        task = self._create_task_entity(db, payload)
        db.commit()
        db.refresh(task)
        return task

    def create_task_from_case(self, db: Session, case: UIAutomationCase, auto_start: bool = True, debug_mode: Optional[bool] = None) -> UIAutomationTask:
        payload = UIAutomationTaskCreate(
            name=case.name,
            case_id=case.id,
            target_url=case.target_url,
            auth_scheme=case.auth_scheme,
            auth_payload=case.auth_payload or {},
            natural_language_steps=case.natural_language_steps or [],
            assertions=case.assertions or [],
            auto_start=auto_start,
            debug_mode=case.debug_mode if debug_mode is None else debug_mode,
        )
        task = self._create_task_entity(db, payload)
        case.last_run_status = task.status
    ...
    def mark_task_running(self, db: Session, task: UIAutomationTask) -> UIAutomationTask:
        ...
        if task.case_id:
            case = self.get_case(db, task.case_id)
            if case:
        ...
        def persist_file_artifact(task_id_value: int, artifact_type: str, artifact_name: str, artifact_bytes: bytes, description: str) -> None:
            ...
            db.add(
                UIAutomationArtifact(
                    ...
                    artifact_type=artifact_type,
                    artifact_name=artifact_name,
                    artifact_path=f"/backend/ui_artifacts/task_{task_id_value}/{artifact_name}",
